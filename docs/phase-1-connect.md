...
Based on the above encouraging results, we have gone on to set up 802.11s on Raspbian. The steps are super simple, and it's [scripted out](https://github.com/tomeshnet/prototype-cjdns-pi2/blob/master/scripts/install.sh). Start by flashing Raspbian Jessie, SSH in and expand your filesystem to fill the available space on your SD card using `sudo raspi-config`, then run:
