...
while true; do
    ...
    if [ "$(which cjdroute)" ]; then
        ...
        while IFS= read -r line; do
            peers+=( "$line" )
        done < <( sudo nodejs /opt/cjdns/tools/peerStats 2>/dev/null | awk '{ if ($3 == "ESTABLISHED") print $2 }' | awk -F. '{print $6".k"}' )
    ...
    if [ "$(which yggdrasil)" ]; then       
        ...
        while IFS= read -r line; do
            peers+=( "$line" )
        done < <( sudo yggdrasilctl getPeers | grep -v "(self)" | awk '{print $1}' | grep -v bytes_recvd )
        for peer in "${peers[@]}"; do
            # shellcheck disable=SC2102
