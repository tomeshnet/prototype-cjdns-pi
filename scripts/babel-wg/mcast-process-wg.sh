...
if [[ "$A" == "WG" ]]; then
    ...
    if [ -z "$(wg 2>&1  |  grep "${peerPub}")" ]; then
        ...
        wg set wg0 listen-port 1010 private-key /etc/wg.key peer $peerPub endpoint [$SOCAT_PEERADDR%$IFACE]:1010 persistent-keepalive 60 allowed-ips fe80::${ip}/128,ff02::1:6/128,40::/8
    fi
fi
